...
class InitApp(object):
    ...
    def set(self):
        ...
        if self.init_app:
            ...
            if not check_if_empty:
                ...
                DataBase(db=self.db, auth=self.auth, request=self.request, tables=['all'])
    ...
    def _load_html_pages(self):
        import os
        for filename in os.listdir(self.request.folder + '/' + 'private/email_templates/'):
            path = self.request.folder + '/' + 'private/email_templates/' + filename
            ...
            if template_key == "verify_email" or template_key == 'reset_password':
                self.db.t_email_template.insert(f_template_key=template_key, f_html_text=XML(str(open(path).read())),
                                              f_plain_text="Auto created by the app",
